...
resource "docker_container" "coredns" {
  ...
  volumes {
    container_path = "/etc/coredns/db.sandbox.local"
...
resource "local_file" "corefile" {
  ...
  content  = file("${path.module}/templates/corefile.tftpl")
}

resource "local_file" "zonefile" {
  filename = local.coredns_zonefile_path
  content  = templatefile("${path.module}/templates/zonefile.tftpl", { local_ip = var.local_ip })
}
